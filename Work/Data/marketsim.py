...
# marketsim.py
#
# Product market simulator.  This simulator creates product market
# data and provides it in several different ways:
#
#    1. Makes periodic updates to a log file productlog.dat
#    2. Provides product data through an embedded HTTP server.
#
...
# Read the product history file as a list of lists
...
class MarketSimulator(object):
    ...
    def add_history(self,filename):
        ...
        for record in hist:
            if record[0] not in self.products:
                self.products[record[0]] = ProductTrack(record[0])
            self.products[record[0]].add_data(record) 

    def reset(self,time):
        self.time = time
        for s in self.products.values():
            s.reset(time)

    # Run forever.  Dt is in seconds
    def run(self,dt):
        for s in self.products:
            self.prices[s] = self.products[s].price
            self.publish(self.products[s].make_record())
        while self.time < 1000:
            for s in self.products:
                self.products[s].incr(dt/60.0)    # Increment is in minutes
                if self.products[s].price != self.prices[s]:
                    self.prices[s] = self.products[s].price
                    self.publish(self.products[s].make_record())
            time.sleep(dt)
            self.time += (dt/60.0)
...
m.register(LogPrinter("marketlog.csv"))
